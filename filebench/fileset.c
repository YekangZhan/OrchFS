...
fileset_mkdir(char *path, int mode)
{
	...
	char *dirs[65536];
	int i = 0;
	// fprintf(stderr,"fileset_mkdir path=%s\n",path);
	if ((p = strdup(path)) == NULL)
		goto null_str;
	// fprintf(stderr,"fileset_mkdir 150 p=%s\n",p);
	/*
	...
	while (1) {
		...
		if (stat64(p, &sb) == 0)
		{
			// fprintf(stderr,"fileset_mkdir 161\n");
			break;
		}
			
		if (strlen(p) < 3)
		{
			// fprintf(stderr,"fileset_mkdir 167\n");
			break;
		}
		...
		// fprintf(stderr,"fileset_mkdir 175\n");
		(void) trunc_dirname(p);
		// fprintf(stderr,"fileset_mkdir 177 p=%s\n",p);
		i++;
	}

	/* Make the directories, from closest to root downwards. */
	for (--i; i >= 0; i--) {
		// fprintf(stderr,"fileset_mkdir 183 i=%d dirs[i]=%s\n",i,dirs[i]);
		(void) FB_MKDIR(dirs[i], mode);
		// fprintf(stderr,"fileset_mkdir 185 i=%d dirs[i]=%s\n",i,dirs[i]);
		free(dirs[i]);
	}
	// fprintf(stderr,"fileset_mkdir 186 p=%s\n",p);
	free(p);
	// fprintf(stderr,"fileset_mkdir 187\n");
	return (FILEBENCH_OK);

null_str:
	/* clean up */
	// fprintf(stderr,"fileset_mkdir 183\n");
	for (--i; i >= 0; i--)
		free(dirs[i]);
...
fileset_create_subdirs(fileset_t *fileset, char *filesetpath)
{
	...
	char *part_path;
	// fprintf(stderr,"fileset_create_subdirs 213 filesetpath=%s\n",filesetpath);
	...
	while (direntry) {
		...
		// fprintf(stderr,"fileset_create_subdirs 220\n");
		// fprintf(stderr,"fileset_create_subdirs part_path=%s\n",part_path);
		// fprintf(stderr,"fileset_create_subdirs full_path=%s\n",full_path);
		free(part_path);
		// fprintf(stderr,"fileset_create_subdirs 223\n");
		/* now create this portion of the subdirectory tree */
		if (fileset_mkdir(full_path, 0755) == FILEBENCH_ERROR)
			return (FILEBENCH_ERROR);
		// fprintf(stderr,"fileset_create_subdirs 227\n");
...
fileset_create(fileset_t *fileset)
{
	...
	if (!reusing) {
		...
		fprintf(stderr,"Pre-allocating 1063\n");
		(void) FB_MKDIR(path, 0755);
		fprintf(stderr,"Pre-allocating 1065\n");
		if (fileset_create_subdirs(fileset, path) == FILEBENCH_ERROR)
			return (FILEBENCH_ERROR);
	}
	//fprintf(stderr,"fileset_create 1054\n");
	start = gethrtime();
