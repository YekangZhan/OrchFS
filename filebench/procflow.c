...
procflow_createproc(procflow_t *procflow)
{
	...
	if (pid == 0) {
		...
		char syscmd[1024];
#endif
		//fprintf(stderr,"procflow_createproc 106\n");
...
#else
		//fprintf(stderr,"procflow_createproc 127\n");
		if (execlp(execname, procname, "-a", procname, "-i",
		    instance, "-s", shmaddr, "-m", shmpath, NULL) < 0) {
			//fprintf(stderr,"procflow_createproc 130\n");
			filebench_log(LOG_ERROR,
			    "procflow exec proc failed: %s",
			    strerror(errno));
			filebench_shutdown(1);
...
#endif
		//fprintf(stderr,"procflow_createproc 137\n");
		exit(1);
	} else {
		/* if parent, save pid and return */
		procflow->pf_pid = pid;
	}
	//fprintf(stderr,"procflow_createproc 142\n");
...
procflow_create_all_procs(void)
{
	...
	while (procflow) {
		...
		for (i = 0; (i < instances) && (ret == 0); i++) {
			...
			//fprintf(stderr,"procflow_create_all_procs 177\n");
			/*
			...
			if (!newproc) {
				...
				//fprintf(stderr,"procflow_create_all_procs 189\n");
				clear_flag(&newproc->pf_threads_defined_flag);
				ret = procflow_createproc(newproc);
				//fprintf(stderr,"procflow_create_all_procs ret=%d\n",ret);
			}
		}
	...
	while (procflow) {
		...
		//fprintf(stderr,"procflow_create_all_procs 215\n");
		procflow = procflow->pf_next;
	}
	//fprintf(stderr,"procflow_create_all_procs 218 ret=%d\n",ret);
	return (ret);
}
