...
uint64_t run_test(test_frame_t * frame){
    ...
    if(frame->mode==SEQREAD)
    {
        for(long long i = 0; i < frame->thd_op_blks; i++)
        {
            ...
            pread(fd, readbuf, rw_size_eachtime[frame->tid][now_arr_cur], add_size);
...
int main(int argc, const char* argv[]){
    ...
    {
        // fprintf(stderr,"%d %s\n",i,file_path[i]);
        create_file(file_path[i], avg_blk_size, MAX_THD_OP_BLKS, i);
    }
    ...
    for(int work_type = 0; work_type < 6; work_type++)
    {
        for(int j = 0; j < 8; j++)
        {
            sleep(3);
            const char* cmd = "sudo echo 3 > /proc/sys/vm/drop_caches";
            system(cmd);
