...
#define ARR_SIZE (20LL*1024*1024)
...
int latancy_per_rw[25001000],latancy_per_r[25001000],latancy_per_w[25001000];
// int rw_size_eachtime[2][20000000];
...
void init_pos_and_type(int64_t* startpos, int64_t* op_type_arr, int op_times, int op_mode, int op_size)
{
    for(long long i = 0; i < op_times; i++)
    {
        ...
        // startpos[i] = myrand() % ((op_times*1LL*op_size-op_size*6) / 10);
    }
...
int create_file(char* filename,int64_t blk_size,int64_t this_file_blk_sum,int tid0)
{
    ...
    // // 保证全在SSD中
    // int64_t total_file_size = blk_size * this_file_blk_sum;
    // int64_t new_blk_size = 64*1024;
    // int64_t new_fblk_num = total_file_size / new_blk_size + 1;
    // char* buf = valloc(2*new_blk_size);
    // long long add0 = 0;
    // int fd = open(filename ,O_RDWR | O_CREAT | O_SYNC , S_IRWXU );
    // fprintf(stderr,"%lld %d\n",new_fblk_num,fd);
    // for(long long i = 0; i < new_fblk_num; i++)
    // {
    //     // write(fd,buf,blk_size);
    //     pwrite(fd,buf,new_blk_size,add0);
    //     // printf("rw_size:%llu\n",rw_size_eachtime[tid0][i]);
    //     // pwrite(fd,buf,blk_size,add0);
    //     add0 += new_blk_size;
    //     // add0 += blk_size;
...
{
    int64_t ret_size = 0, muti = 200; //1==50M
    if(work_threads <= 4)
...
void get_rw_size_eachtime(int64_t avg_blk_size, int64_t RW_BLKS,int threads)
{
    ...
    int64_t size1k = 1024, size8k = 8*1024, size32k = 32*1024, size128k = 128*1024, size512k = 512*1024;
    for(int i = 0; i <= threads; i++)
    {
        for(int64_t j = 0; j <= RW_BLKS+10; j+=2)
        {
            if(avg_blk_size==size1k)
            {
                int64_t randsize = myrand()%((int64_t)(size1k*0.2))+(int64_t)(size1k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size1k*2-randsize;
            }
            else if(avg_blk_size==size4k)
            {
                int64_t randsize = myrand()%((int64_t)(size4k*0.2))+(int64_t)(size4k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size4k*2-randsize;
            }
            else if(avg_blk_size==size8k)
            {
                int64_t randsize = myrand()%((int64_t)(size8k*0.2))+(int64_t)(size8k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size8k*2-randsize;
            }
            else if(avg_blk_size==size16k)
            {
                int64_t randsize = myrand()%((int64_t)(size16k*0.2))+(int64_t)(size16k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size16k*2-randsize;
            }
            else if(avg_blk_size==size32k)
            {
                int64_t randsize = myrand()%((int64_t)(size32k*0.2))+(int64_t)(size32k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size32k*2-randsize;
            }
            else if(avg_blk_size==size64k)
            {
                int64_t randsize = myrand()%((int64_t)(size64k*0.2))+(int64_t)(size64k*0.9);
                // int64_t randsize = myrand()%(size64k*2)+1;
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size64k*2-randsize;
            }
            else if(avg_blk_size==size128k)
            {
                int64_t randsize = myrand()%((int64_t)(size128k*0.2))+(int64_t)(size128k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size128k*2-randsize;
            }
            else if(avg_blk_size==size256k)
            {
                int64_t randsize = myrand()%((int64_t)(size256k*0.2))+(int64_t)(size256k*0.9);
                // int64_t randsize = myrand()%(size256k*2)+1;
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size256k*2-randsize;
            }
            else if(avg_blk_size==size512k)
            {
                int64_t randsize = myrand()%((int64_t)(size512k*0.2))+(int64_t)(size512k*0.9);
                rw_size_eachtime[i][j] = randsize;
                rw_size_eachtime[i][j+1] = size512k*2-randsize;
            }
            else
            {
